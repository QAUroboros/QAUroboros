<h1 align="center">Всем привет, я Артём </h1>

Я — **ручной тестировщик** с богатым опытом ручного тестирования, и сейчас начинаю свой путь в **автоматизированном тестировании**.  
Моя цель — обеспечить высочайшее качество продукта, постоянно изучая и применяя современные инструменты и методологии.

## Мой технологический стек

<div align="center">
  <img src="https://resources.jetbrains.com/storage/products/pycharm/img/meta/pycharm_logo_300x300.png" alt="PyCharm" heigth="40" width="40"/>
  <img src="https://upload.wikimedia.org/wikipedia/commons/2/29/Pytest_logo.png" alt="PyTest" heigth="40" width="40"/>
  <img src="https://about.gitlab.com/images/press/logo/png/gitlab-icon-rgb.png" alt="GitLab" heigth="40" width="40"/>
  <img src="https://upload.wikimedia.org/wikipedia/commons/e/e2/BrowserStack_Logo.png" alt="BrowserStack" heigth="40" width="40"/>
</div>

## Обо мне

Я убеждён, что качественное тестирование — это основа успешного продукта.  
Мой опыт ручного тестирования позволил мне глубоко понять процессы разработки, и теперь я стремлюсь интегрировать автоматизированные решения для оптимизации тестирования.  
Постоянно совершенствуюсь, изучая Python и современные библиотеки для автоматизации, чтобы внедрять лучшие практики в каждую задачу.

## Контакты
- **Linkedin:**[Linkedin](https://www.linkedin.com/in/krivoshein-artem/)
- **Email:** [krivo6ein@gmai.com](krivo6ein@gmai.com)
- **GitHub:** [GitHub](https://github.com/QAUroboros)
- **CV:** [hh.kz](https://hh.kz/resume/1193a03eff08968eb80039ed1f775976333743)[**Хабр Карьера**](https://career.habr.com/artemkqa)

